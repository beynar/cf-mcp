import { MaybePromise, OmitNever } from './types';
...
export type HandlePromptPayload<Schema extends StandardSchemaV1 | undefined> = (payload: PromptPayload<Schema>) => MaybePromise<{
	description?: string;
	role?: 'assistant' | 'user';
	text: string;
}>;
...
export class Prompt<Schema extends StandardSchemaV1 | undefined, H extends HandlePromptPayload<Schema>> {
	...
	'~call' = (input: Schema extends StandardSchemaV1 ? StandardSchemaV1.InferInput<Schema> : undefined, sessionId: string) => {
		return this['~handler']({ input, sessionId, error } as PromptPayload<Schema>);
	};
